...
function gerarProdutos() {
    ...
    sanduiches.forEach(produto => {
        ...
        let collapseId = `collapse-${produto.nome.replace(/\s+/g, '-')}`; 
        col.innerHTML = `
...
function gerarAdicionais() {
    const containerAdicionais = document.getElementById('adicionais');
        ...
        let collapseId = `collapse-${adicional.nome.replace(/\s+/g, '-')}`; 
...
function gerarHotDogs() {
    ...
    let collapseId = `collapse-${hotdog.nome.replace(/\s+/g, '-')}`;
...
function gerarPasteis() {
    const containerPasteis = document.getElementById('pasteis');
        ...
        let collapseId = `collapse-${pastel.nome.replace(/\s+/g, '-')}`; 
...
function gerarBebidas() {
    ...
    bebidas.forEach(bebida => {
        ...
        let collapseId2 = `collapse-${bebida.nome.replace(/\s+/g, '-')}`;
        col.innerHTML = `
            <div class="card h-100">
                <div class="card-body">
...
function atualizarListaCarrinho() {
    ...
    if (totalPromocao > 0) {
        const pares = Math.floor(totalPromocao / 2); 
        const restantes = totalPromocao % 2; 
        precoPromocao = (pares * 6.50 + restantes * 3.50).toFixed(2);
...
function atualizarSubtotal() {
    ...
    for (const produto in produtosSelecionados) {
        ...
        if (produto === 'BOHEMIA PURO MALTE 300ML' || 
            ...
            subtotal += pares * 6.50 + restantes * 3.50; 
        } else {
            subtotal += quantidade * precosProdutos[produto]; 
        }
    }
...
function finalizarCompra() {
    ...
    const numeroWhatsApp = '5534998320622'; 
    const url = `https://api.whatsapp.com/send?phone=5534998320622&text=${encodeURIComponent(mensagem)}`;
